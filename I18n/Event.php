...
class Event
{
    ...
    * @param string | DateTime $startDate = when does this event 
     * start
    ...
    */
    ...
    {
        // init vars
        $this->id = md5($title . $interval . $value) . 
        ...
        try {
            $this->interval = new DateInterval(sprintf($interval,
                    $value));
        } catch (Exception $e) {
            error_log($e->getMessage());
            throw new InvalidArgumentException(
                    self::ERROR_INVALID_INTERVAL);
        }
        // store / calc dates
    ...
    */
    ...
    /**
    * This is used if $occurrences is set, so we'll know the 
    * end date
    * 
    ...
    */
    protected function calcEndDateFromOccurrences()
    {
        ...
        $period = new DatePeriod($this->startDate, 
                $this->interval, $this->occurrences);
    ...
    * @return boolean FALSE if past end date || DateTime 
    * $nextDate
    */
