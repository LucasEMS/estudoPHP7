...
class Convert 
{
    const EXCEPTION_FILE_NOT_EXIST = 'File does not exist';
    
    /**
     * Scans $filename
     * 
     * Rewrites code when possible
     * Otherwise adds "// WARNING: xxx" before offernding lines
     * 
        ...
        $result = preg_replace_callback_array( 
                ...
                '!(.*?)\$\$!' =>
                    function ($match) {
                        return '// WARNING: variable interpolation'
                                ...
                                . '// migration70.incompatible.php' . PHP_EOL
                                . $match[0];
                    },
                            
                // changer in how the list() operator is handled
                '!(.*?)list(\s*?)?\(!' =>
                    function ($match) {
                ...
                '!(.*?)set_error_handler(\s*?)?.*\(!' =>
                    function ($match) {
                        return '// WARNING: might not '
                                . 'catch all errors'
                                . '// see: http://php.net/manual/en/'
                                . '// language.error.php7.php' . PHP_EOL
                                . $match[0];
                    },
                    ...
                    function ($match) {
                        if (isset($match[3])) {
                            ...
                            . 'affects the handler assigned by '
                                    ...
                                    . 'where ignore_usre_ablort(0 is TRUE '
                                    . 'has been fixed in PHP 7.' . PHP_EOL
                                    . 'This could potentially break '
                                    . 'your code under '
                                    . 'certain circumstance.' . PHP_EOL
                                    . 'See: http://php.net/manual/en/'
                                    . 'migration70.incompatible.php' . PHP_EOL
                                    . $match[0];
                        } else {
            ...
            //                           1     2      3
             '!call_user_method_array\((.*?),(.*?)\)(\b|;)!' =>
                function ($match) {
                    return '// WARNUNG: call_user_method_array()'
            ...
            // PHP 5: preg_replace('/[0-9]{1,3}/e', 'sprintf("%03d",$ids)
            // PHP 7: preg_replace('/[0-9]{1,3}/', function ($m) {return
            //                          sprintf("%03d", $m); }, $ids)
